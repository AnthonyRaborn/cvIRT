# LOOCV wrappers around TAM's EM algorithm.
#
# Standard tam.mml/tam.mml.2pl crash on single-observation data because
# tam_mml_proc_unidim_simplify computes sd(Y) which is NA for n=1.
# These wrappers bypass that step and override maxKi to use the full
# dataset's category counts rather than the (possibly degenerate) test
# subset's counts.
#
# The 2PL wrapper also skips the slope M-step since B is held fixed.

tam.mml.loocv <- function(resp, Y = NULL, group = NULL, irtmodel = "1PL",
                          maxKiInput, formulaY = NULL, dataY = NULL, ndim = 1,
                          pid = NULL, xsi.fixed = NULL, xsi.inits = NULL,
                          beta.fixed = NULL, beta.inits = NULL,
                          variance.fixed = NULL, variance.inits = NULL,
                          est.variance = TRUE, constraint = "cases",
                          A = NULL, B = NULL, B.fixed = NULL, Q = NULL,
  ...
  if (constraint == "items") irtmodel <- "PCM2"
  if ((irtmodel == "PCM2") & is.null(A)) {
    A <- TAM:::.A.PCM2(resp, constraint = constraint, Q = Q)
  }
  if (!is.null(con$seed)) set.seed(con$seed)

  nitems <- ncol(resp)
  if (is.null(colnames(resp))) colnames(resp) <- paste0("I", 100 + 1:nitems)
  nstud <- nrow(resp)
  nstud1 <- sum(1 * (rowSums(1 - is.na(resp)) > 0))
  if (is.null(pweights)) pweights <- rep(1, nstud)
  ...
  res <- TAM:::tam_mml_progress_proc_nodes(progress = progress, snodes = snodes,
                                           nnodes = nnodes, skillspace = "normal",
                                           QMC = QMC)

  maxK <- max(resp, na.rm = TRUE) + 1
  modeltype <- if (irtmodel == "RSM") "RSM" else "PCM"

  # Use caller-supplied maxKi instead of computing from resp
  maxKi <- maxKiInput

  design <- TAM:::designMatrices(modeltype = modeltype, maxKi = maxKi,
                                 resp = resp, A = A, B = B, Q = Q, R = R,
                                 ndim = ndim, constraint = constraint)
  A <- design$A
  B <- design$B
  cA <- design$flatA
  cA[is.na(cA)] <- 0
  ...
  G <- res$G; groups <- res$groups; group <- res$group; var.indices <- res$var.indices

  res <- TAM:::tam_mml_inits_beta(Y = Y, formulaY = formulaY, dataY = dataY,
                                  G = G, group = group, groups = groups,
                                  nstud = nstud, pweights = pweights,
                                  ridge = ridge, beta.fixed = beta.fixed,
                                  xsi.fixed = xsi.fixed, constraint = constraint,
                                  ndim = ndim, beta.inits = beta.inits)
  Y <- res$Y; nullY <- res$nullY; formulaY <- res$formulaY; nreg <- res$nreg
  W <- res$W; YYinv <- res$YYinv; beta.fixed <- res$beta.fixed; beta <- res$beta

  res <- TAM:::tam_mml_proc_response_indicators(resp = resp, nitems = nitems)
  resp <- res$resp; resp.ind <- res$resp.ind
  resp.ind.list <- res$resp.ind.list; nomiss <- res$nomiss

  AXsi <- matrix(0, nrow = nitems, ncol = maxK)

  res <- TAM:::tam_mml_proc_xsi_parameter_index_A(A = A, np = np)
  indexIP <- res$indexIP; indexIP.list <- res$indexIP.list
  indexIP.list2 <- res$indexIP.list2; indexIP.no <- res$indexIP.no

  res <- TAM:::tam_mml_sufficient_statistics(nitems = nitems, maxK = maxK,
                                             resp = resp, resp.ind = resp.ind,
                                             pweights = pweights, cA = cA,
                                             progress = progress)
  ItemScore <- res$ItemScore; cResp <- res$cResp; col.index <- res$col.index

  res <- TAM:::tam_mml_inits_xsi(A = A, resp.ind = resp.ind, ItemScore = ItemScore,
                                 xsi.inits = xsi.inits, xsi.fixed = xsi.fixed,
                                 est.xsi.index = est.xsi.index, pweights = pweights,
                                 xsi.start0 = xsi.start0, xsi = xsi, resp = resp)
  xsi <- res$xsi; personMaxA <- res$personMaxA
  ItemMax <- res$ItemMax; equal.categ <- res$equal.categ

  prior_list_xsi <- TAM:::tam_mml_proc_prior_list_xsi(
    prior_list_xsi = prior_list_xsi, xsi = xsi)

  xsi.min.deviance <- xsi; beta.min.deviance <- beta
  variance.min.deviance <- variance

  res <- TAM:::tam_mml_create_nodes(snodes = snodes, nodes = nodes,
                                    ndim = ndim, QMC = QMC)
  theta <- res$theta; theta2 <- res$theta2; thetawidth <- res$thetawidth
  theta0.samp <- res$theta0.samp; thetasamp.density <- res$thetasamp.density

  deviance <- 0
  deviance.history <- TAM:::tam_deviance_history_init(maxiter = maxiter)
  iter <- 0; a02 <- a1 <- 999; a4 <- 0
  se.xsi <- 0 * xsi; se.B <- 0 * B

  # Bypass unidim_simplify — crashes on n=1 due to sd(Y)==NA
  unidim_simplify <- FALSE; YSD <- FALSE; Avector <- FALSE

  res <- TAM:::tam_mml_warning_message_multiple_group_models(ndim = ndim, G = G)
  res <- TAM:::tam_acceleration_inits(acceleration = acceleration, G = G,
                                      xsi = xsi, variance = variance)
  ...
  hwt.min <- 0; rprobs.min <- 0; AXsi.min <- 0; B.min <- 0
  deviance.min <- 1e+100; itemwt.min <- 0
  se.xsi.min <- se.xsi; se.B.min <- se.B

  # EM loop
  while (((!betaConv | !varConv) | ((a1 > conv) | (a4 > conv) | (a02 > convD))) &
         (iter < maxiter)) {
    iter <- iter + 1
    res <- TAM:::tam_mml_progress_em0(progress = progress, iter = iter, disp = disp)

    if (snodes > 0) {
      res <- TAM:::tam_mml_update_stochastic_nodes(
        theta0.samp = theta0.samp, variance = variance,
        snodes = snodes, beta = beta, theta = theta)
      theta <- res$theta; theta2 <- res$theta2
    ...
    res <- TAM:::tam_mml_calc_prob(iIndex = 1:nitems, A = A, AXsi = AXsi,
                                   B = B, xsi = xsi, theta = theta,
                                   nnodes = nnodes, maxK = maxK, recalc = TRUE,
                                   maxcat = maxcat, use_rcpp = FALSE)
    rprobs <- res$rprobs; AXsi <- res$AXsi
    ...
    res.hwt <- TAM:::tam_calc_posterior(rprobs = rprobs, gwt = gwt,
                                        ...
                                        normalization = TRUE,
                                        thetasamp.density = thetasamp.density,
                                        snodes = snodes, resp.ind = resp.ind,
                                        avoid.zerosum = TRUE)
    hwt <- res.hwt$hwt

    resr <- TAM:::tam_mml_mstep_regression(
      resp = resp, hwt = hwt, resp.ind = resp.ind, pweights = pweights,
      pweightsM = pweightsM, Y = Y, theta = theta, theta2 = theta2,
      YYinv = YYinv, ndim = ndim, nstud = nstud, beta.fixed = beta.fixed,
      variance = variance, Variance.fixed = variance.fixed, group = group,
      G = G, snodes = snodes, nomiss = nomiss,
    ...
    beta <- resr$beta; variance <- resr$variance; itemwt <- resr$itemwt
    variance_acceleration <- resr$variance_acceleration
    variance_change <- resr$variance_change; beta_change <- resr$beta_change
    ...
    res <- TAM:::tam_mml_mstep_intercept(
      ...
      prior_list_xsi = prior_list_xsi,
      mstep_intercept_method = mstep_intercept_method,
      n.ik = n.ik, maxcat = maxcat)
    xsi <- res$xsi; se.xsi <- res$se.xsi
    max.increment <- res$max.increment
    xsi_acceleration <- res$xsi_acceleration
    xsi_change <- res$xsi_change; logprior_xsi <- res$logprior_xsi

    res <- TAM:::tam_mml_compute_deviance(
    ...
    deviance <- res$deviance; deviance.history <- res$deviance.history
    a01 <- rel_deviance_change <- res$rel_deviance_change
    a02 <- deviance_change <- res$deviance_change; penalty_xsi <- res$penalty_xsi
    if (con$dev_crit == "relative") a02 <- a01

    if (deviance < deviance.min) {
      xsi.min.deviance <- xsi; beta.min.deviance <- beta
      variance.min.deviance <- variance; hwt.min <- hwt
      AXsi.min <- AXsi; B.min <- B; deviance.min <- deviance
      itemwt.min <- itemwt; se.xsi.min <- se.xsi; se.B.min <- se.B
    }

    a1 <- xsi_change; devch <- -(deviance - olddeviance)
    res <- TAM:::tam_mml_progress_em(
      ...
      variance_change = variance_change, B_change = 0, devch = devch,
      penalty_xsi = penalty_xsi)
  }

  # Restore best-deviance estimates
  xsi <- xsi.min.deviance; beta <- beta.min.deviance
  variance <- variance.min.deviance; hwt <- hwt.min
  AXsi <- AXsi.min; B <- B.min; deviance <- deviance.min
  itemwt <- itemwt.min; se.xsi <- se.xsi.min; se.B <- se.B.min

  AXsi <- TAM:::tam_mml_include_NA_AXsi(AXsi = AXsi, maxcat = maxcat)
  xsi.fixed.estimated <- TAM:::tam_generate_xsi_fixed_estimated(xsi = xsi, A = A)
  B.fixed.estimated <- TAM:::tam_generate_B_fixed_estimated(B = B)
  se.AXsi <- TAM:::tam_mml_se_AXsi(AXsi = AXsi, A = A, se.xsi = se.xsi,
                                    maxK = maxK)
  ic <- TAM:::tam_mml_ic(nstud = nstud1, deviance = deviance, xsi = xsi,
                         xsi.fixed = xsi.fixed, beta = beta,
                         beta.fixed = beta.fixed, ndim = ndim,
                         variance.fixed = variance.fixed, G = G,
                         irtmodel = irtmodel, B_orig = NULL,
                         B.fixed = B.fixed, E = E, est.variance = TRUE,
                         resp = resp, est.slopegroups = NULL,
  ...
  res <- tam_calc_counts_single(resp = resp, theta = theta,
                                resp.ind = resp.ind, group = group,
                                maxK = maxK, pweights = pweights, hwt = hwt)
  n.ik <- res$n.ik; pi.k <- res$pi.k

  item1 <- TAM:::tam_itempartable(resp = resp, maxK = maxK, AXsi = AXsi,
                                  B = B, ndim = ndim, resp.ind = resp.ind,
                                  rprobs = rprobs, n.ik = n.ik, pi.k = pi.k)

  res <- TAM:::tam_mml_person_posterior(pid = pid, nstud = nstud,
                                        pweights = pweights, resp = resp,
                                        resp.ind = resp.ind, snodes = snodes,
                                        hwtE = hwt, hwt = hwt, ndim = ndim,
                                        theta = theta)
  person <- res$person; EAP.rel <- res$EAP.rel
  s2 <- Sys.time()

  if (is.null(dimnames(A)[[3]])) dimnames(A)[[3]] <- paste0("Xsi", 1:dim(A)[3])
  ...
  res.hwt <- TAM:::tam_calc_posterior(rprobs = rprobs, gwt = 1 + 0 * gwt,
                                      resp = resp, nitems = nitems,
                                      resp.ind.list = resp.ind.list,
                                      normalization = FALSE,
                                      thetasamp.density = thetasamp.density,
  ...
  latreg_stand <- TAM:::tam_latent_regression_standardized_solution(
    variance = variance, beta = beta, Y = Y)
  deviance.history <- deviance.history[1:iter, ]

  res <- list(
    xsi = xsi, beta = beta, variance = variance, item = item1,
    person = person, pid = pid, EAP.rel = EAP.rel, post = hwt,
    rprobs = rprobs, itemweight = itemwt, theta = theta, n.ik = n.ik,
    pi.k = pi.k, Y = Y, resp = resp0, resp.ind = resp.ind, group = group,
    ...
    time = list(start = s1, end = s2, diff = s2 - s1), A = A, B = B, se.B = se.B,
    ...
    B.fixed.estimated = B.fixed.estimated, beta.fixed = beta.fixed,
    Q = Q, variance.fixed = variance.fixed, nnodes = nnodes,
    deviance = ic$deviance, ic = ic,
    thetasamp.density = thetasamp.density,
...
tam.mml.2pl.loocv <- function(resp, Y = NULL, group = NULL, irtmodel = "2PL",
                              formulaY = NULL, dataY = NULL, ndim = 1,
                              pid = NULL, xsi.fixed = NULL, xsi.inits = NULL,
                              maxKiInput, beta.fixed = NULL, beta.inits = NULL,
                              variance.fixed = NULL, variance.inits = NULL,
                              est.variance = FALSE, A = NULL, B = NULL,
                              B.fixed = NULL, Q = NULL, est.slopegroups = NULL,
                              E = NULL, gamma.init = NULL, pweights = NULL,
                              userfct.variance = NULL, variance.Npars = NULL,
  ...
  if (!is.null(variance.fixed)) est.variance <- TRUE
  ...
  design <- TAM:::designMatrices(modeltype = "PCM", maxKi = maxKi,
                                 resp = resp, A = A, B = B, Q = Q,
                                 R = R, ndim = ndim)
  A <- design$A; B <- design$B
  cA <- design$flatA; cA[is.na(cA)] <- 0
  if (progress) {
    cat("    * Created Design Matrices   (", paste(Sys.time()), ")\n")
    utils::flush.console()
  }
  design <- NULL; B_orig <- B
  ...
  res <- TAM:::tam_mml_inits_beta(Y = Y, formulaY = formulaY, dataY = dataY,
                                  ...
                                  nstud = nstud, pweights = pweights,
                                  ridge = ridge, beta.fixed = beta.fixed,
                                  xsi.fixed = xsi.fixed, constraint = "cases",
                                  ndim = ndim, beta.inits = beta.inits)
  Y <- res$Y; nullY <- res$nullY; formulaY <- res$formulaY; nreg <- res$nreg
  W <- res$W; YYinv <- res$YYinv; beta.fixed <- res$beta.fixed; beta <- res$beta
  ...
  if (irtmodel == "GPCM.design") {
    ...
    if (!is.null(gamma.init)) basispar1 <- gamma.init
    for (dd in 1:ndim) basispar[, dd] <- basispar1
  }
  ...
  # Bypass unidim_simplify — crashes on n=1
  unidim_simplify <- FALSE; YSD <- FALSE; Avector <- FALSE

  res <- TAM:::tam_acceleration_inits(acceleration = acceleration, G = G,
                                      xsi = xsi, variance = variance,
                                      B = B, irtmodel = irtmodel)
  xsi_acceleration <- res$xsi_acceleration
  variance_acceleration <- res$variance_acceleration
  ...
  deviance.min <- 1e+100; itemwt.min <- 0
  se.xsi.min <- se.xsi; se.B.min <- se.B
  B_change <- 0; xsi_change <- 0
  mpr <- round(seq(1, np, len = 10))

  # EM loop — xsi and B are fixed, only population params update
  while (((!betaConv | !varConv) | ((a1 > conv) | (a4 > conv) | (a02 > convD))) &
         (iter < maxiter)) {
    iter <- iter + 1
    res <- TAM:::tam_mml_progress_em0(progress = progress, iter = iter, disp = disp)
    ...
    resr <- TAM:::tam_mml_mstep_regression(
      ...
      YYinv = YYinv, ndim = ndim, nstud = nstud, beta.fixed = beta.fixed,
      variance = variance, Variance.fixed = variance.fixed, group = group,
      G = G, snodes = snodes, thetasamp.density = thetasamp.density,
    ...
    res <- TAM:::tam_mml_2pl_sufficient_statistics_item_slope(
      hwt = hwt, theta = theta, cResp = cResp, pweights = pweights,
      maxK = maxK, nitems = nitems, ndim = ndim)
    thetabar <- res$thetabar; cB_obs <- res$cB_obs; B_obs <- res$B_obs

    # M-step intercept and slope skipped: xsi and B are held fixed for LOOCV

    res <- TAM:::tam_mml_compute_deviance(
      loglike_num = res.hwt$rfx, loglike_sto = res.hwt$rfx,
      snodes = snodes, thetawidth = thetawidth, pweights = pweights,
      deviance = deviance, deviance.history = deviance.history, iter = iter)
    deviance <- res$deviance; deviance.history <- res$deviance.history
    a01 <- rel_deviance_change <- res$rel_deviance_change
    ...
    res <- TAM:::tam_mml_progress_em(
      ...
      rel_deviance_change = rel_deviance_change,
      xsi_change = xsi_change, beta_change = beta_change,
      variance_change = variance_change, B_change = B_change, devch = devch)
  }
  ...
  AXsi <- TAM:::tam_mml_include_NA_AXsi(AXsi = AXsi, maxcat = maxcat)
  xsi.fixed.estimated <- TAM:::tam_generate_xsi_fixed_estimated(xsi, A)
  B.fixed.estimated <- TAM:::tam_generate_B_fixed_estimated(B)
  se.AXsi <- TAM:::tam_mml_se_AXsi(AXsi = AXsi, A = A, se.xsi = se.xsi,
                                    maxK = maxK)

  ic <- TAM:::tam_mml_ic(nstud = nstud100, deviance = deviance, xsi = xsi,
                         xsi.fixed = xsi.fixed, beta = beta,
                         beta.fixed = beta.fixed, ndim = ndim,
                         variance.fixed = variance.fixed, G = G,
                         irtmodel = irtmodel, B_orig = B_orig,
                         B.fixed = B.fixed, E = E, est.variance = est.variance,
                         resp = resp, est.slopegroups = est.slopegroups,
                         variance.Npars = variance.Npars, group = group)
...
tam_calc_counts_single <- function(resp, theta, resp.ind, group, maxK,
                                   pweights, hwt) {
  TP <- nrow(theta)
  I <- ncol(resp)
  ...
  list(n.ik = n.ik, pi.k = prob.theta)
}
